...
async function calculate() {
    ...
    try {
        ...
        if (N < 1) throw new Error("Number of turns must be at least 1.");

        let p_mm;
        if (pitchType === 'identical') {
            const p0 = parseFloat(document.getElementById('p0').value);
            if (isNaN(p0)) throw new Error("Pitch must be a valid number.");
            p_mm = Array(N > 1 ? N - 1 : 0).fill(p0);
        } else {
            p_mm = document.getElementById('pList').value.split(',').map(v => parseFloat(v.trim()));
            if (p_mm.some(isNaN)) throw new Error("All comma-separated pitches must be valid numbers.");
            if (N > 1 && p_mm.length !== N - 1) {
                 throw new Error(`Please provide exactly ${N - 1} comma-separated pitch values for ${N} turns.`);
            }
        }
        ...
        if (shape === 'spiral') {
            ...
            if (radii.some(r => r <= 0)) throw new Error("Pitch is too large; inner radius is zero or negative.");
        ...
        container.innerHTML = `<div class="result-grid"></div>`; // Clear previous results
        ...
        const container = document.getElementById('result-container');
        container.innerHTML = `<div class="placeholder-state"><p>Error: ${error.message}</p></div>`;
    } finally {
